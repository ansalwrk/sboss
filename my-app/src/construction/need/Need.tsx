...
function Need() {
  const [displayTitle, setDisplayTitle] = useState(
    <div>
      ...
      <div className="needp cldull pt-5">
        <h4 className="pb-5">Built for Real-World Project Execution</h4>
  ...
  const [activeCard, setActiveCard] = useState<number | null>(1); // Set to 1 to match default state
  ...
  return (
    <div className='allbg pb-20 md:pb-80'>
      {/* Header Section */}
      <div className='grid grid-cols-1 pt-20 md:pt-60 pb-10 px-6' style={{ textAlign: "center" }}>
        <p className='clmg' style={{ fontSize: "16px", fontWeight: "650" }}>
          EVERYTHING YOU NEED
        </p>
        <h2 className='clwt' style={{ fontSize: "clamp(32px, 5vw, 48px)", fontWeight: "700" }}>
      ...
      <div className="flex flex-col items-center justify-center px-4 md:px-0">
        ...
        {/* Tabs Container: Responsive flex wrap */}
        <div className='flex flex-wrap justify-center gap-2 pb-10 max-w-6xl'>
          {cards.map((card) => (
            <div
              key={card.id}
              className={`btnneed cursor-pointer btn11 transition-all duration-300 text-sm md:text-base ${activeCard === card.id ? 'active-card' : ''}`}
        ...
        {/* Main Content Area: grid-cols-1 on mobile, grid-cols-2 on laptop */}
        <div className='grid grid-cols-1 md:grid-cols-2 px-4 md:px-13 w-full max-w-7xl'>
          
          <div className='cardbig pt-10 md:pt-15 pl-6 md:pl-12 pr-6 pb-10' 
               style={{ borderRadius: window.innerWidth > 768 ? "15px 0px 0px 15px" : "15px 15px 0px 0px" }}>
            
            <div className='clwt pt-2 pb-4' style={{ fontSize: "clamp(24px, 4vw, 30px)", fontWeight: "700" }}>
              {displayTitle}
            </div>
            ...
            <div className='clwt pt-2' style={{ fontSize: "30px", fontWeight: "700" }}>
              {displayAlltext}
            </div>

            <p className='flex flex-row items-center cldull pt-6' style={{ fontSize: "12px", fontWeight: "400" }}>
              <Shield className="clmg mr-2" size={16} /> Self-hosted and secure • Your data stays with you
            </p>
          </div>

          <div className='cardbig overflow-hidden' 
               style={{ borderRadius: window.innerWidth > 768 ? "0px 15px 15px 0px" : "0px 0px 15px 15px" }}>
            <img className='needimg w-full h-full object-cover' src={Needimg} alt="Need illustration" />
          </div>
        ...
        <div className="grid grid-cols-1 place-items-center mt-10">
          <p className="cardsm items-center flex clmg" style={{ fontSize: "13px", fontWeight: '600' }}>
            <Shield size={16} className="mr-2" />100% Self-Hosted
          </p>
        </div>
      </div>
    </div>
